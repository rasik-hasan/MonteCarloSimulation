...
def plotFrequencyBarT(freqDict,fdict ):
    ...
    lists2 = sorted(fdict.items(), key=operator.itemgetter(0), reverse=False)[0:10]
    ...
    #plt.xticks(r,np.arange(min(counts), max(counts) + 1, 200))
    plt.xticks(r,terms)
    # This shows bars for the Y value instead of points, and aligns each bar to the center of each X value.
    plt.plot(r, counts, label="standard robot")
    plt.plot(r,counts2, label="random robot")
